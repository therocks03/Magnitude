...
// Mobile Menu functions for the new navigation
function toggleMenu() {
...
function exportPDF() {
    ...
    // Give user brief visual feedback
    const exportBtn = document.getElementById('btn-export-pdf');
    const originalText = exportBtn.innerHTML;
    exportBtn.innerHTML = '<i class="ph ph-spinner ph-spin"></i> Processing...';
    
    // Create an invisible form to submit the HTML content to the backend for PDF rendering
    const form = document.createElement('form');
    form.method = 'POST';
    form.action = 'http://localhost:3000/api/export-pdf';
    
    const inputHtml = document.createElement('input');
    inputHtml.type = 'hidden';
    inputHtml.name = 'html';
    // We send only the interior results cards so the backend can wrap it cleanly
    inputHtml.value = resultsContainer.innerHTML; 
    
    form.appendChild(inputHtml);
    document.body.appendChild(form);
    
    // Submitting the native form triggers a standard browser download, avoiding typical "Blob" heuristicts
    form.submit();
